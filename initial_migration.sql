/*
  ============================================================================
  MAURIPLAY INITIAL MIGRATION
  ============================================================================

  Complete database schema initialization for MauriPlay digital marketplace.

  This migration builds the entire database from scratch including:
  ...
  SAFE TO RUN: Uses IF NOT EXISTS to prevent conflicts with existing objects
  ...
  TABLES:
  1. platforms - Gaming and service platforms
  2. products - Digital products for each platform
  3. inventory - Digital codes/keys inventory
  4. users - User accounts with PIN authentication and wallet
  5. payment_methods - Available payment methods for manual payments
  6. verification_codes - OTP codes for phone verification
  7. orders - Purchase orders from users
  8. wallet_topups - Wallet top-up requests
  9. wallet_transactions - Audit trail for wallet changes
  10. settings - Application configuration

  ENUMS:
  - inventory_status: available, reserved, pending_approval, sold, returned, compromised
  - user_role: admin, user
  - payment_type: wallet, manual
...
COMMENT ON TABLE platforms IS 'Gaming and service platforms available in marketplace';
...
COMMENT ON TABLE inventory IS 'Digital codes and keys inventory';
...
COMMENT ON TABLE users IS 'User accounts with PIN authentication and wallet system';
...
-- Inventory indexes (FIFO ordering and status queries)
...
-- Orders indexes (user queries and admin dashboard)
...
-- Wallet transactions indexes (user history)
...
CREATE INDEX IF NOT EXISTS idx_wallet_transactions_topup ON wallet_transactions(topup_id);

-- Wallet topups indexes
CREATE INDEX IF NOT EXISTS idx_wallet_topups_user_status ON wallet_topups(user_id, status);
CREATE INDEX IF NOT EXISTS idx_wallet_topups_status_created ON wallet_topups(status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_wallet_topups_payment_method_id ON wallet_topups(payment_method_id);
...
-- =====================================================
-- 5.2 APPLY TIMESTAMP TRIGGERS TO ALL TABLES
-- =====================================================
...
-- =====================================================
-- 5.3 LIMIT PENDING TOPUPS TRIGGER
-- =====================================================
...
-- =====================================================
-- 5.4 SETTINGS UPDATED_AT TRIGGER
-- =====================================================
...
-- =====================================================
-- 6.6 VERIFICATION_CODES POLICIES
-- =====================================================
...
-- =====================================================
-- 6.8 WALLET_TRANSACTIONS POLICIES
-- =====================================================
...
-- ============================================================================
-- SECTION 9: ACCOUNT RECOVERY FUNCTIONS
-- ============================================================================
...
-- ============================================================================
-- SECTION 12: ORDER MANAGEMENT FUNCTIONS
-- ============================================================================
...
-- =====================================================
-- 13.1 RECEIPTS BUCKET (Public - for order receipts)
-- =====================================================
...
-- =====================================================
-- 13.2 WALLET-RECEIPTS BUCKET (Private - for wallet topup receipts)
-- =====================================================

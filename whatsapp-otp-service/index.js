...
// If you protect your service with a token, set WHATSAPP_OTP_SERVICE_TOKEN
// and send `Authorization: Bearer <token>` from callers (e.g. Supabase Edge Function).
const SERVICE_TOKEN = process.env.WHATSAPP_OTP_SERVICE_TOKEN || '';

app.use(express.json());

// Optional auth guard for server-to-server calls (does not break existing API if unset).
app.use((req, res, next) => {
  if (!SERVICE_TOKEN) return next();
  // Only protect API endpoints (leave dashboard public)
  if (req.path === '/' || req.path.startsWith('/api/')) return next();

  const auth = String(req.headers.authorization || '');
  if (auth === `Bearer ${SERVICE_TOKEN}`) return next();
...
app.post('/send-otp', async (req, res) => {
  try {
    const { phone, otp: externalOtp } = req.body || {};
    ...
    // If an OTP is provided (e.g. generated by Supabase), we only send it.
    // Otherwise we generate and store OTP in memory as before.
    const otp = externalOtp ? String(externalOtp) : generateOtp();
    if (!externalOtp) {
      saveOtp(digits, otp);
    }

    // Keep existing behavior: add delay before sending
